...
class WaterPredictor:
    ...
    def predict_any_site(self, site_history, target_point, target_year, target_quarter):
        ...
        warning = f"警告：数据量过少（仅{site_count}条），结果为外推" if site_count < self.WARN_DATA_COUNT else ""
        ...
        try:
            ...
            # 🛠️ 关键修复 1：强制转换为 np.float32 满足 TensorFlow 的严苛要求
            X_seq_input = np.asarray(recent_data[self.target_features].values, dtype=np.float32).reshape(1, self.LOOKBACK_DAYS, len(self.target_features))
            
            spatial_features = ['sea_area_encoded', 'province_encoded', 'city_encoded', 'point_location_encoded', 'lon_norm', 'lat_norm']
            spatial_vals = np.asarray(recent_data.iloc[-1][spatial_features].values, dtype=np.float32)
            ...
            time_norm = np.asarray(self.scalers['time'].transform(time_df)[0], dtype=np.float32)
            ...
            # 🛠️ 关键修复 2：黑匣子记录仪，把真正的错误代码显示在表格里！
            ...
            return {
                ...
                '警告信息': f"⚠️ 错误追踪: {str(e)}", '_norm_preds': fallback_norm
            }
            ...
            for year in years:
                for q in quarters:
                    ...
                    if len(site_history) > 0:
                        ...
                        # 🛠️ 关键修复 3：防止时间转换器报错
                        time_df_new = pd.DataFrame([[year, {1:4, 2:7, 3:10}[q], q]], columns=['year', 'month', 'quarter'])
                        time_norm_new = self.scalers['time'].transform(time_df_new)[0]
                        new_row['year_norm'], new_row['month_norm'], new_row['quarter_norm'] = time_norm_new
